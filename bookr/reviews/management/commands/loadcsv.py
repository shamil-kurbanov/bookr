...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for data_dict in models.get('Publisher', []):
            p, created = reviews.models.Publisher.objects.get_or_create(name=data_dict['publisher_name'], defaults={
        ...
        for data_dict in models.get('Book', []):
            b, created = reviews.models.Book.objects.get_or_create(title=data_dict['book_title'], defaults={
                'publication_date': data_dict['book_publication_date'].replace('/', '-'),
                'isbn': data_dict['book_isbn'],
                'publisher': reviews.models.Publisher.objects.get(name=data_dict['book_publisher_name'])
            })
        ...
        for data_dict in models.get('Contributor', []):
            c, created = reviews.models.Contributor.objects.get_or_create(first_name=data_dict['contributor_first_name'],
                                                                          last_name=data_dict['contributor_last_name'],
                                                                          email=data_dict['contributor_email'])

            if created:
                print('Created Contributor "{} {}"'.format(data_dict['contributor_first_name'],
                                                           data_dict['contributor_last_name']))

        for data_dict in models.get('BookContributor', []):
            book = reviews.models.Book.objects.get(title=data_dict['book_contributor_book'])
            contributor = reviews.models.Contributor.objects.get(email=data_dict['book_contributor_contributor'])
            bc, created = reviews.models.BookContributor.objects.get_or_create(book=book,
        ...
        for data_dict in models.get('Review', []):
            ...
            book = reviews.models.Book.objects.get(title=data_dict['review_book'])

            review, created = reviews.models.Review.objects.get_or_create(content=data_dict['review_content'],
                                                                                book=book,
                                                                                creator=creator,
                                                                                defaults={
